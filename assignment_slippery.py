    ...
    while True:
        for state in range(nS):
            ...
            for i in range(len(P[state][action])):
                expected_return += P[state][action][i][0]*prev_value_function[P[state][action][i][1]]
            value_function[state] = P[state][action][0][2] + gamma*expected_return
        #print('this is value function',value_function)  
        diff = 0    
        for state in range(nS):
            if (abs(value_function[state] - prev_value_function[state]) > diff):
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    ############################
    # YOUR IMPLEMENTATION HERE #
    value_from_action = np.zeros((nS,nA))
    ...
    for state in range(nS):
        ...
        for action in range(nA):
            expected_return = 0
            for i in range(len(P[state][action])):
                expected_return += P[state][action][i][0]*value_from_policy[P[state][action][i][1]]
            # print('this is value from action',value_from_action)
            # print('this is value from policy',value_from_policy)
            value_from_action[state][action] = P[state][action][0][2] + gamma*expected_return
            if(value_from_action[state][action]>=max_value):
                max_value = value_from_action[state][action]
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    while(status == 1):
        ...
        if (i>10):
            if(np.array_equal(np.array(value_function), np.array(old_value_function)) == True):
                status = 0
    ...
    while(status == 1):
        for state in range(nS):
            ...
            for action in range(nA):
                expected_return = 0
                for i in range(len(P[state][action])):
                    expected_return = expected_return + P[state][action][i][0]*(prev_value_function[P[state][action][i][1]] + P[state][action][i][2])
                #print("expected returnnnnnnnnnn",expected_return)
                value_function[state] = gamma*expected_return 
                if(value_function[state]>max_val):
                    max_val = value_function[state]
                    new_policy[state] = action
            value_function[state] = max_val
        #print(policy, new_policy)
        if k>5:
                if(np.array_equal(np.array(policy), np.array(new_policy)) == True):
                    status = 0
        k = k+1
